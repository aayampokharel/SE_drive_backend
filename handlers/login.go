...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logInFailure = functions.SetErrorModel(http.StatusBadRequest, "Invalid JSON format. Invalid LogIn") //set error models sets the error model , nothing else .
		json.NewEncoder(w).Encode(logInFailure)

		//!returns errorsmodel in response.
		return
	}
